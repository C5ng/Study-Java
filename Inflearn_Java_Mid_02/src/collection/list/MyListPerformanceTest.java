...
public class MyListPerformanceTest {
    public static void main(String[] args) {
        ...
        search(arrayList, loop, 0); // 앞 검색
        search(arrayList, loop, size / 2); // 중간 검색
        search(arrayList, loop, size - 1); // 마지막 검색

        System.out.println("==MyLinkedList 검색==");
        search(linkedList, loop, 0); // 앞 검색
        search(linkedList, loop, size / 2); // 중간 검색
        search(linkedList, loop, size - 1); // 마지막 검색
    }
